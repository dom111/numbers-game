...
- For daily puzzles, the first hint level is free. Escalating to the operator hint (and anything beyond it for
  that in-progress step) counts as one hint used.
...
- Daily completion stores move count, shortest-path length, star rating, and hint count so restored daily wins keep the same summary.
...
- Shared daily text includes stars, move count vs best, and the number of hints used.
